...
function kyc.auth_check( req )
    ...
    db.query( "DELETE FROM kyc_session WHERE tscreate + INTERVAL '1 DAY' < NOW()" )
    ...
    local res = db.query( [[SELECT * FROM kyc_session WHERE access = ?
    ...
    if #res == 0 then
        ...
        if client and client.active then
            ...
            --    else
            --        db.query( 'INSERT INTO users (userid,name,email) VALUES (?,?,?)',
            --            client.client_id, client.fullname, client.email )
            --    end
            --end

            --Проверяем наличие какой-либо записи в БД по данному токену
            local session = db.query( 'SELECT * FROM kyc_session WHERE access = ?', token )

            --Если есть, то ставим время проверки на сейчас
            if #session > 0 then
                db.query( 'UPDATE kyc_session SET tsupdate = NOW() WHERE access = ?', token )
            --Иначе
            else
                ...
                db.query( [[INSERT INTO kyc_session (access,tscreate,tsupdate,expires_in,client)
                    VALUES (?,TO_TIMESTAMP(?),NOW(),?,?)]], token, client.iat, expires_in, client.client_id )
            end
        ...
        --Запрос не успешен или учетка не активна.
            ...
            if #session > 0 then
                db.query( 'DELETE FROM kyc_session WHERE access = ?', token )
            end
